...
(defn get-patient
 [id-data]
  ;; (select-keys (db/patient-by-id db/spec id-data) [:fullname :sex :address :insurance :birthdate]))
  (let [p (select-keys (db/patient-by-id db/spec id-data) [:fullname
                                                           :sex
                                                           :address
                                                           :insurance
                                                           :birthdate])]
    (if (empty? p) {} (assoc p :birthdate (format-date (:birthdate p))))))
